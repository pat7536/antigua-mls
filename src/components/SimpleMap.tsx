...
import { getCoordinatesFromLocation, getLocationDisplayName } from '@/lib/locationMapping';
...
export default function SimpleMap({ properties = [] }: SimpleMapProps) {
  ...
  const addPropertyMarkers = (L: any) => {
    ...
    console.log(`Map: ${markersAdded} markers added, ${mappingsFailed} properties couldn't be mapped (${properties.length} total)`);
  };
    ...
    if (typeof window !== 'undefined' && mapRef.current && !mapInstanceRef.current) {
      let timeoutId = setTimeout(() => {
        import('leaflet').then((L) => {
          ...
          // Initialize map
          mapInstanceRef.current = L.map(mapRef.current!).setView([17.1274, -61.8468], 11);
  ...
  useEffect(() => {
    if (!mapInstanceRef.current || typeof window === 'undefined' || properties.length === 0) {
      return;
    }
