...
export const groupPropertiesByLocation = (
  properties: Property[]
...
export const calculateLocationStats = (
  ...
  const validSqFtData = properties
    .filter(
      (p) =>
        p.fields.Price && p.fields.SquareFootage && p.fields.SquareFootage > 0
    )
    .map((p) => ({
  ...
  const avgPricePerSqFt =
    validSqFtData.length > 0
      ? Math.round(
          validSqFtData.reduce((sum, data) => sum + data.price / data.sqft, 0) /
            validSqFtData.length
        )
      : 0;
...
export const getFastestMovingRegions = (
  trends: MarketTrend[],
  limit: number = 5
): MarketTrend[] => {
  return trends
...
export const calculateOverallMarketStats = (
  properties: Property[]
): {
  ...
  const avgPricePerSqFt =
    validSqFtData.length > 0
      ? Math.round(
          validSqFtData.reduce((sum, price) => sum + price, 0) /
            validSqFtData.length
        )
      : 0;
  ...
  const avgPriceChange =
    trends.length > 0
      ? Math.round(
          trends.reduce((sum, trend) => sum + trend.priceChangePercent, 0) /
            trends.length
        )
      : 0;
